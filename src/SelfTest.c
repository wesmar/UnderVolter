// SelfTest.c — Power-management stability self-test: dispatches the AVX2
//              ComboHell stress kernel across all logical processors and reports
//              the error count.  The kernel exercises mixed integer (VPXOR) and
//              floating-point (VRCPPS/VDPPS) YMM register operations.
#include <PiPei.h>
#include <Uefi.h>
...
#include "./ASMx64/CpuStressorAvx2.h"
#include "SelfTest.h"
...
// Input data layout for the ComboHell AVX2 kernel.  Each segment maps to a
// contiguous range of YMM registers loaded at kernel entry:
//   seg1 → YMM0–YMM7  (256-bit integer patterns)
//   seg2 → YMM8–YMM10 (32-bit floats, used by VRCPPS/VDPPS)
//   seg3 → YMM11–YMM14 (more integer patterns for VPXOR chains)
...
// EFI_AP_PROCEDURE callback: entry point for each logical CPU during the test.
// Simply calls RunAvx2StressKernel with the shared input/output scratch buffers.
VOID EFIAPI PM_ComboHell_Thread(IN OUT VOID* Buffer)
{
  //
  // Run ASM kernel
  
  RunAvx2StressKernel( (void*)&combo_scratch1, (void*)&combo_scratch2 );
}
...
// Configure the AVX2 stressor globals, launch PM_ComboHell_Thread on all
// processors (BSP inline, APs via EFI_MP_SERVICES), and print the final
// error count.  Error detection relies on the kernel reading back its YMM
// register outputs and comparing against expected values.
EFI_STATUS RunPowerManagementSelfTest(VOID)
{
  ...
  UiAsciiPrint("[SelfTest] Running %u Iterations of ComboHell_AVX2 Stressor\n",
    gSelfTestMaxRuns);

  RunOnAllProcessors(PM_ComboHell_Thread, TRUE, NULL);

  UiAsciiPrint( "Self test completed with %u errors.\n", 
    gSelfTestErrorCnt);
