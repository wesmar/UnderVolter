// MiniLog.c — Per-CPU diagnostic overlay rendered directly to the GOP
//             framebuffer (no UEFI console, no libc).  Each logical core is
//             assigned a fixed Y-stripe based on its AbsIdx so traces from
//             concurrent AP threads never collide.  Protected by a ticket-lock.
//             Entire module is compiled out when ENABLE_MINILOG_TRACING is
//             not defined; all public macros expand to nothing in that case.
#include <PiPei.h>
#include <Uefi.h>
...
static volatile UINT32 gMiniTraceTicket = 0;
static volatile UINT32 gMiniTraceServing = 0;

// Acquire the trace lock using a ticket-lock: atomically claim the next ticket,
// then spin (with PAUSE) until gMiniTraceServing reaches our ticket number.
static VOID MiniTraceLock(VOID)
{
  UINT32 ticket = AtomicIncrementU32((UINT32*)&gMiniTraceTicket) - 1;
  while (gMiniTraceServing != ticket) {
    _mm_pause();
  }
}

// Release the trace lock by advancing the serving counter.
static VOID MiniTraceUnlock(VOID)
{
  AtomicIncrementU32((UINT32*)&gMiniTraceServing);
}
...
// Render ASCII string s at pixel (x,y) using the embedded SSFN2 font, writing
// 32-bit colour values directly into the linear framebuffer (lfb).
// CR resets x to 0; LF advances y by font->fbHeight.
...
// Locate the GOP, switch to mode 0 (firmware default), and cache the
// framebuffer address, width, height, and pitch.  Sets haveConsole to TRUE
// only if a non-zero framebuffer was successfully mapped.
...
// Render a structured trace entry for a low-level HW operation (MSR/MMIO).
// Format: [PKG#][CORE#][ns] - <opName> : param1 : param2
// Each logical core writes to its own fixed stripe (2*AbsIdx+1 lines from top)
// so concurrent AP traces do not overwrite each other.
void MiniTrace(
               const UINT8  operId,
...
// Render a printf-style trace line for arbitrary diagnostic messages.
// Format: [PKG#][CORE#][ns] <formatted message>
// Writes to the even stripe (2*AbsIdx) for this core, one line above MiniTrace.
void MiniTraceEx(
