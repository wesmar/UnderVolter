// TurboRatioLimits.c — Read/write wrappers for MSR_TURBO_RATIO_LIMIT (0x1AD)
//                      and MSR_TURBO_RATIO_LIMIT_ECORE (0x650), plus a heuristic
//                      all-core ratio programmer that avoids touching zero/0xFF
//                      blocks (which indicate unused or locked core-count slots).
#include <Uefi.h>
...
// Return raw MSR_TURBO_RATIO_LIMIT (0x1AD): eight 8-bit per-core-count ratio
// slots packed into one UINT64, byte[0]=1-core max, byte[7]=8-core max.
...
// Return raw MSR_TURBO_RATIO_LIMIT_ECORE (0x650); same byte layout as 0x1AD.
...
// Write val to MSR_TURBO_RATIO_LIMIT (0x1AD) and verify with a readback
// after a 10 µs stall.  Returns EFI_INVALID_PARAMETER if readback mismatches.
...
/*******************************************************************************
 * SetEfficientCoreTurboRatioLimits
 ******************************************************************************/

// Write val to MSR_TURBO_RATIO_LIMIT_ECORE (0x650) with the same write-then-
// readback verification as SetTurboRatioLimits.
...
// Set all apparently-active byte slots in MSR_TURBO_RATIO_LIMIT to maxRatio.
// Slots with value 0 or 0xFF are treated as unused/invalid and left untouched.
...
// Same as ProgramMaxTurboRatios but targets MSR_TURBO_RATIO_LIMIT_ECORE (0x650).
EFI_STATUS ProgramEfficientCoreMaxTurboRatios(const UINT8 maxRatio)
{
  ...
  return SetEfficientCoreTurboRatioLimits(msr);
}